...
// Fade-up Reveal
const fadeElements = document.querySelectorAll('.project-card, .approach-card, .faq-item, .contact-card, .tool-item, .about-grid, .agency-card, .result-card, .minimal-card, .reveal');
...
if (marqueeTrack && marqueeTrack.children.length === 0) {
    ...
    const webflowSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 63" style="width:24px; height:auto;"><path fill-rule="evenodd" clip-rule="evenodd" d="M100 0L68.0913 62.3782H38.12L51.4737 36.526H50.8746C39.8578 50.8273 23.4206 60.2417 0 62.3782V36.8837C0 36.8837 14.9827 35.9988 23.7906 26.7386H0V0.000492722H26.7381V21.9922L27.3382 21.9897L38.2643 0.000492722H58.4857V21.8527L59.0858 21.8518L70.4219 0H100Z" fill="#a855f7"/></svg>`;
    let html = '';
    for (let i = 0; i < 3; i++) {
        toolsList.forEach(t => {
            if (t.type === 'svg') html += `<span class="tool-item">${webflowSVG} ${t.name}</span>`;
            else html += `<span class="tool-item"><i class="${t.icon}"></i> ${t.name}</span>`;
        });
    }
    ...
    function animateMarquee() {
        if (!isHovering && marqueeTrack) {
            pos -= speed;
            if (Math.abs(pos) >= marqueeTrack.scrollWidth / 3) pos = 0;
    ...
    sections.forEach(section => {
        ...
        if (scrollPosition >= sectionTop && scrollPosition < sectionBottom) {
            navItems.forEach(item => {
                ...
                if (href === `#${sectionId}` || (href && href.includes(`#${sectionId}`))) item.classList.add('active');
            });
        }
    });
}
